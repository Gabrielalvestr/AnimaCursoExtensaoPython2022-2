# Meu Primeiro projeto Python!!!
#
# print() = comando de saida 
print("Alo Mundo!")

# Quando quiser guardar uma String! (frases)
nome = "Gabriel Alves"
#Quando quiser guardar um numeor interiro
idade = 19
valor = 40
endereco = "rua mooca paulista"
email = "lokipo@teste.com.br"

#Exibir o meu nome (que esta dentro da variavel nome)
print(nome)
print(idade)
print(valor)
print(endereco)
print(email)
#Quando Quiser exibir a frase "Minha idade é " completando  com o conteudo da variavel idade
print("Meu nome é "+nome)
print ("Minha idade é "+str(idade))
print(f"Minha idade é {idade}\n")
print("Minha idade é {}".format(idade))
print("valor a pagar é "+str(valor))
print("o endereco é "+endereco)
print("qual o seu email: "+email)


#Quando quiser exibir "Meu nome é ... e tenho ... anos.." trocando pelas variaveis nome e idade 
print("Meu nome é {} e tenho {} anos".format(nome,idade))

'''
Comentario em bloco
'''
#teste 
